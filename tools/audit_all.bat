...
REM --- Fix navigation to project root (robust) ---
set SCRIPT_DIR=%~dp0
pushd "%SCRIPT_DIR%\.."
...
REM --- 2. Full .bss symbols (use symbol type B/b) ---
echo [2/9] Creating nm_bss_all.txt
arm-none-eabi-nm --print-size --size-sort --radix=d "%ELF%" | findstr " B " > "%OUTDIR%\nm_bss_all.txt"

REM --- 3. Full .data symbols (use symbol type D/d) ---
echo [3/9] Creating nm_data_all.txt
arm-none-eabi-nm --print-size --size-sort --radix=d "%ELF%" | findstr " D " > "%OUTDIR%\nm_data_all.txt"

REM --- 4. Global RAM Top (200 largest symbols overall) ---
...
echo [9/9] Creating audit_headers.txt
