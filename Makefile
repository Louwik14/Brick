...
HOST_SEQ_RUNNER_SMOKE_TEST := $(HOST_TEST_DIR)/seq_runner_smoke_tests
HOST_SEQ_RUNNER_PLOCK_ROUTER_TEST := $(HOST_TEST_DIR)/seq_runner_plock_router_tests
HOST_SEQ_RUNNER_PLOCK_ROUTER_POOL_TEST := $(HOST_TEST_DIR)/seq_runner_plock_router_pool_tests
...
HOST_SEQ_PLOCK_POOL_TEST := $(HOST_TEST_DIR)/seq_plock_pool_tests
HOST_SEQ_PLOCK_WRITE_POOL_TEST := $(HOST_TEST_DIR)/seq_plock_write_pool_tests
HOST_SEQ_READER_PLOCK_ITER_TEST := $(HOST_TEST_DIR)/seq_reader_pl_iter_tests
...
check-host: $(CHECK_HOST_TARGETS)
	...
	@echo "Running P-lock pool tests"
	$(HOST_SEQ_PLOCK_POOL_TEST)
	@echo "Running pooled p-lock write tests"
	$(HOST_SEQ_PLOCK_WRITE_POOL_TEST)
	@echo "Running PLK2 roundtrip test"
	$(HOST_SEQ_PLK2_ROUNDTRIP_TEST)
	@echo "Running PLK2 mini-fuzz test"
	$(HOST_SEQ_PLK2_MINIFUZZ_TEST)
	@echo "Running host hold/runtime bridge tests"
	$(HOST_SEQ_HOLD_TEST)
	@echo "Running live capture sanity test"
	$(HOST_SEQ_LIVE_REC_SANITY_TEST)
	...
	@echo "Running reader p-lock iterator tests"
	$(HOST_SEQ_READER_PLOCK_ITER_TEST)
	...
	@echo "Running runner smoke test"
	$(HOST_SEQ_RUNNER_SMOKE_TEST)
	@echo "Running runner p-lock router legacy test"
...
$(HOST_SEQ_PLOCK_POOL_TEST): tests/seq_plock_pool_tests.c core/seq/seq_plock_pool.c core/seq/seq_plock_pool.h
	@mkdir -p $(HOST_TEST_DIR)
...
$(HOST_SEQ_PLK2_ROUNDTRIP_TEST): tests/plk2_roundtrip.c core/seq/seq_model.c core/seq/seq_model_consts.c core/seq/seq_project.c core/seq/seq_runtime.c $(HOST_SEQ_RUNTIME_SRCS) $(SEQ_LED_BRIDGE_HOLD_SLOTS_STUB) cart/cart_registry.c tests/stubs/board_flash_stub.c
	@mkdir -p $(HOST_TEST_DIR)
	$(HOST_CC) $(HOST_CFLAGS) -Itests/stubs -I. -Icore -Icart -Iboard \
	tests/plk2_roundtrip.c core/seq/seq_model.c core/seq/seq_model_consts.c core/seq/seq_project.c core/seq/seq_runtime.c $(HOST_SEQ_RUNTIME_SRCS) $(SEQ_LED_BRIDGE_HOLD_SLOTS_STUB) cart/cart_registry.c tests/stubs/board_flash_stub.c -o $@

$(HOST_SEQ_PLK2_MINIFUZZ_TEST): tests/plk2_minifuzz.c core/seq/seq_model.c core/seq/seq_model_consts.c core/seq/seq_project.c core/seq/seq_runtime.c $(HOST_SEQ_RUNTIME_SRCS) $(SEQ_LED_BRIDGE_HOLD_SLOTS_STUB) cart/cart_registry.c tests/stubs/board_flash_stub.c
	@mkdir -p $(HOST_TEST_DIR)
	$(HOST_CC) $(HOST_CFLAGS) -Itests/stubs -I. -Icore -Icart -Iboard \
	tests/plk2_minifuzz.c core/seq/seq_model.c core/seq/seq_model_consts.c core/seq/seq_project.c core/seq/seq_runtime.c $(HOST_SEQ_RUNTIME_SRCS) $(SEQ_LED_BRIDGE_HOLD_SLOTS_STUB) cart/cart_registry.c tests/stubs/board_flash_stub.c -o $@
...
$(HOST_SEQ_LIVE_REC_SANITY_TEST): tests/live_rec_sanity.c core/seq/seq_live_capture.c core/seq/seq_model.c core/seq/seq_model_consts.c core/seq/seq_project.c core/seq/seq_runtime.c $(HOST_SEQ_RUNTIME_SRCS) $(SEQ_LED_BRIDGE_HOLD_SLOTS_STUB) cart/cart_registry.c tests/stubs/board_flash_stub.c
	@mkdir -p $(HOST_TEST_DIR)
	$(HOST_CC) $(HOST_CFLAGS) -DSEQ_PLOCK_POOL_CAPACITY_TEST=9 -Itests/stubs -I. -Icore -Icart -Iboard \
tests/live_rec_sanity.c core/seq/seq_live_capture.c core/seq/seq_model.c core/seq/seq_model_consts.c core/seq/seq_project.c core/seq/seq_runtime.c $(HOST_SEQ_RUNTIME_SRCS) $(SEQ_LED_BRIDGE_HOLD_SLOTS_STUB) cart/cart_registry.c tests/stubs/board_flash_stub.c -o $@
...
$(HOST_SEQ_READER_TEST): tests/seq_reader_tests.c core/seq/seq_model.c core/seq/seq_model_consts.c core/seq/seq_project.c core/seq/seq_runtime.c $(HOST_SEQ_RUNTIME_SRCS) $(SEQ_LED_BRIDGE_HOLD_SLOTS_STUB)
	@mkdir -p $(HOST_TEST_DIR)
...
$(HOST_SEQ_RT_TIMING_TEST): tests/seq_rt_timing_tests.c $(HOST_SEQ_RUNTIME_SRCS) core/seq/seq_runtime.c core/seq/seq_project.c core/seq/seq_model.c core/seq/seq_model_consts.c cart/cart_registry.c board/board_flash.c $(SEQ_LED_BRIDGE_HOLD_SLOTS_STUB)
	@mkdir -p $(HOST_TEST_DIR)
	$(HOST_CC) $(HOST_CFLAGS) -Itests/stubs -I. -Icore -Icart -Iboard \
	tests/seq_rt_timing_tests.c $(HOST_SEQ_RUNTIME_SRCS) core/seq/seq_runtime.c core/seq/seq_project.c core/seq/seq_model.c core/seq/seq_model_consts.c cart/cart_registry.c board/board_flash.c $(SEQ_LED_BRIDGE_HOLD_SLOTS_STUB) -o $@
...
$(HOST_SEQ_LED_SNAPSHOT_TEST): tests/seq_led_snapshot_tests.c core/seq/seq_runtime.c core/seq/seq_project.c core/seq/seq_model.c core/seq/seq_model_consts.c cart/cart_registry.c $(HOST_SEQ_RUNTIME_SRCS) tests/stubs/board_flash_stub.c $(SEQ_LED_BRIDGE_HOLD_SLOTS_STUB)
	@mkdir -p $(HOST_TEST_DIR)
	$(HOST_CC) $(HOST_CFLAGS) -Itests/stubs -I. -Icore -Icart -Iboard \
...
# ------------------------------------------------------------
.PHONY: check_no_engine_anywhere check_no_legacy_includes_apps check_no_ccm_in_apps check_no_cold_refs_in_apps

check_no_engine_anywhere:
	@echo "[CI] check_no_engine_anywhere"
	@if rg -P --no-heading --line-number -g '*.c' -g '*.h' 'seq_engine(?!_runner)' .; then \
		echo "[CI][FAIL] seq_engine remnants found"; exit 1; \
	else \
		echo "[CI][OK] no seq_engine remnants"; \
	fi
