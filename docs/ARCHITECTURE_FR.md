...
**Note P1 (MP5, 2025-10-25)** — Côté `apps/**`, les handles sont activés globalement (`SEQ_USE_HANDLES=1` via `CFLAGS`) et le garde-fou bloquant `check_no_legacy_includes_apps` interdit tout include direct de `seq_project.h` / `seq_model.h`. Les apps ne doivent inclure que `core/seq/seq_access.h` et passer par le Reader (flags `SEQ_STEPF_*`, itérateur p-locks) tandis que le core résout encore les handles en interne. Voir `PROGRESS_P1.md#MP5` pour le journal détaillé.
