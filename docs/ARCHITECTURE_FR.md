...
2. `ui_keyboard_bridge_update_from_model()` synchronise Root/Scale/Omnichord, mais aussi les 20 paramètres du sous-menu Arpégiateur (`apps/ui_arp_menu.c`). Toute modification déclenche `arp_set_config()` ; le flag legacy `KBD_UI_LOCAL_ARP` est tenu à jour pour compatibilité. Le paramètre **Hold** (remplace l'ancien On/Off local) est propagé via `arp_set_hold()` afin de laisser l'arpégiateur tourner après release quand demandé. // --- ARP FIX: Hold ---
3. `sink_note_on()` route soit directement vers `seq_recorder_handle_note_on_at()` + `ui_backend_note_on()` (ARP OFF), soit vers `arp_note_input()` (ARP ON). Les accords omnichord/ARP partagent désormais le même timestamp système (burst ≤1 ms) afin d'éviter les micro-retards entre voix enregistrées. Le moteur planifie swing/strum/repeat et renvoie les notes générées via ses callbacks, qui continuent d'alimenter le recorder avant l'émission MIDI. // --- ARP FIX: timestamp accords ---
