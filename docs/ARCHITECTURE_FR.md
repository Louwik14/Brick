...
* Les **cartouches** (`cart/`) reçoivent leurs p-locks via `cart_link.c` qui manipule un shadow de paramètres et sérialise les trames UART. L'invariant playback reste inchangé : **p-locks avant NOTE_ON**, **NOTE_OFF jamais droppé** et **STOP → CC#123** sont garantis par le runner Reader-only.【F:apps/seq_engine_runner.c†L130-L353】
