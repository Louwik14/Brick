...
static inline bool _step_has_active_voice(const seq_model_step_t *step) {
    if (step == NULL) {
        return false;
    }
...
static void _rebuild_runtime_from_pattern(void) {
    ...
    for (uint8_t local = 0U; local < SEQ_LED_BRIDGE_STEPS_PER_PAGE; ++local) {
        ...
        const bool has_voice = _step_has_active_voice(src);
...
void seq_led_bridge_quick_toggle_step(uint8_t i) {
    ...
    const bool was_on = _step_has_active_voice(step) || (step->plock_count > 0U);
    if (was_on) {
