...
static plk2_decode_result_t decode_plk2_chunk(seq_model_step_t *step,
    ...
    uint8_t target_count = stored_count;
    if (policy == TRACK_LOAD_DIFFERENT_CART) {
        target_count = 0U;
        for (uint8_t i = 0U; i < stored_count; ++i) {
            const uint8_t flags = payload[(size_t)i * 3U + 2U];
            const uint8_t id = payload[(size_t)i * 3U];
            if ((flags & SEQ_READER_PL_FLAG_DOMAIN_CART) != 0U || pl_is_cart(id)) {
                continue;
            }
            target_count++;
        }
    } else if (policy == TRACK_LOAD_ABSENT) {
        target_count = 0U;
    }

    if (target_count > SEQ_MAX_PLOCKS_PER_STEP) {
    ...
    uint16_t offset = 0U;
    if (seq_plock_pool_alloc(target_count, &offset) != 0) {
...
static bool decode_track_steps_plk2(seq_model_track_t *track,
    ...
    for (uint8_t i = 0U; i < SEQ_MODEL_STEPS_PER_TRACK; ++i) {
        seq_model_step_t *step = &track->steps[i];
...
bool seq_pattern_load(uint8_t bank, uint8_t pattern) {
    ...
    if (header.version != SEQ_PROJECT_PATTERN_VERSION) {
        return false;
    }
